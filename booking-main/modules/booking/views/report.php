...
class View extends \Booking\Tools\View
{
    ...
    public function onRow($item, $o, $prop)
    {
        ...
        $item['contact'] .= '<br><a href="tel:'.$item['phone'].'">'.$item['phone'].'</a>';
        $item['name'] = '<span class=two_lines><span class=term style="background-color:'.$item['color'].'">'.$item['name'].'</span>';
        $item['name'] .= '<br><span class=small>'.self::toDate($item).'</span></span>';
        $item['create_date'] = Date::format($item['create_date'], 'd M Y').'<br>{LNG_Time} '.Date::format($item['create_date'], 'H:i');
        $this->topic['topic'] = '<b>'.$item['topic'].'</b><br><span class=small>';
        foreach ($this->category->items() as $k => $v) {
            if (isset($item[$v])) {
                $this->topic[$k] = $this->category->get($k, $item[$v]);
            }
        }
        $item['topic'] = '<span class=two_lines>'.implode(' ', $this->topic).'</span></span>';
