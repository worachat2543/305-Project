...
class View extends \Gcms\View
{
    ...
    public function room($order)
    {
        ...
        foreach (Language::get('ROOM_CUSTOM_TEXT', array()) as $key => $label) {
            $content .= '<tr><th>'.$label.'</th><td>'.(isset($order->{$key}) ? $order->{$key} : '').'</td></tr>';
        }
    ...
    public function booking($order)
    {
        ...
        $content .= '<tr><th class=top>{LNG_Booking date}</th><td>'.\Booking\Tools\View::toDate($order).'</td></tr>';
        // หมวดหมู่
        ...
        $content .= '<tr><th>{LNG_Status}</th><td><span class="term'.$order['status'].'">'.Language::find('BOOKING_STATUS', null, $order['status']).'</span></td></tr>';
        if (!empty($order['approved_date'])) {
            $content .= '<tr><th>{LNG_Approver}</th><td>'.$order['approver_name'].'</td></tr>';
            $content .= '<tr><th>{LNG_Approval date}</th><td>'.Date::format($order['approved_date']).'</td></tr>';
        }
