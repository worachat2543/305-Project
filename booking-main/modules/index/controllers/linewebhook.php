...
class Controller extends \Kotchasan\Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $datas = array(
            ...
            'get' => json_encode($request->getQueryParams(), JSON_UNESCAPED_UNICODE),
            'post' => json_encode($request->getParsedBody(), JSON_UNESCAPED_UNICODE),
            'server' => json_encode($request->getServerParams(), JSON_UNESCAPED_UNICODE),
            'content' => $content
        );
        \Kotchasan\Model::createQuery()
            ->insert('callback', $datas)
            ->execute();
        // userId, type
