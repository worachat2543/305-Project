        ...
        if ($request->initSession() && $request->isSafe() && $login = Login::isMember()) {
            if (Login::checkPermission($login, 'can_config') && Login::notDemoMode($login)) {
                try {
                    ...
                    foreach (array('user_forgot', 'user_register', 'welcome_email', 'member_only', 'demo_mode', 'show_title_logo', 'activate_user') as $key) {
                    ...
                    $config->warpper_bg_color = $request->post('warpper_bg_color')->filter('#ABCDEF0-9');
                    if (empty($ret)) {
                        // อัปโหลดไฟล์
